...
EMG filtering and spectral analysis derived from the original project
notebook. Originally a Jupyter notebook (`cods_protese_emg.ipynb`); now
also maintained as a runnable Python script (`emg_filter_analysis.py`).

## Files

| File                          | What it is                                                            |
|-------------------------------|-----------------------------------------------------------------------|
| `emg_filter_analysis.py`      | Cleaned-up runnable version of the notebook (entry point). Reads `../rec_emg/new_emg_data8.csv`. |
...
5. **Script-relative path for the CSV.** The notebook used
   ...
   `os.path.dirname(__file__)` and reads from the canonical copy at
   `../rec_emg/new_emg_data8.csv`, so it runs from any working directory.
...
7. **Constants pulled to the top.** `fs`, `lowcut`, `highcut`, `notch_freq`,
   and `notch_Q` are grouped together so a single value can be changed
...
- Sampling rate is hard-coded to `fs = 500` Hz, matching how the recording
  was generated by `rasp.py` at the project root.
